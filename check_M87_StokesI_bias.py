...
save_dir = "/home/ilya/github/bk_transfer/vak/bias"
...
if not only_make_pics:
    ...
    for freq in freqs_obs_ghz:
        ...
        #                      scale=scale*(15.4/freq)**(-alpha_true))
        # BK from bk_transfer
        jm.load_image_stokes("I", "{}/jet_image_i_{}.txt".format(jetpol_files_directory, 15.4),
                             scale=scale*(freq/15.4)**alpha_true)
...
itrue_convolved_low_image = create_image_from_fits_file("{}/convolved_true_jet_model_i_rotated_{}.fits".format(save_dir, freq_low))
slice1 = itrue_convolved_low_image.slice(point1=(-8, 6), point2=(-10, -3))

# fig, axes = plt.subplots(1, 1)
# axes.plot(slice1, label="true conv")
...
ipol_low_image = ccimages[freq_low]
slice2 = ipol_low_image.slice(point1=(-8, 6), point2=(-10, -3))
# axes.plot(slice2, label="CLEAN")
# plt.legend()
# plt.show()
...
fig = iplot(ipol_arrays[freq_low], true_convolved_spix_array, x=ccimages[freq_low].x, y=ccimages[freq_low].y,
            min_abs_level=3*std_dict[freq_low], colors_mask=common_imask, color_clim=[alpha_true-0.5, alpha_true+0.5], blc=blc_low, trc=trc_low,
...
fig = iplot(ipol_arrays[freq_low], spix_array - true_convolved_spix_array, x=ccimages[freq_low].x, y=ccimages[freq_low].y,
            min_abs_level=3*std_dict[freq_low], colors_mask=common_imask, color_clim=[-0.5, 0.5], blc=blc_low, trc=trc_low,
            beam=common_beam, close=True, colorbar_label=r"$\alpha$", show_beam=True, show=False,
            cmap='bwr', contour_color='black', plot_colorbar=True,
            contour_linewidth=0.25)
